...
// 返回聚类中心
vector<Point> KMeans::start(std::vector<Point> &points, int num_clusters) {
...
bool KMeans::init(const std::vector<Point> &points) {
  ...
  for (int idx = 0; idx < num_clusters_; ++idx) {
      Point center(points[points_indices[idx]]);
...
bool KMeans::assign() {
  ...
  for (auto &point : points_) {
    ...
    // inline (changed = changed || KFUpdate) since the compiler will
    // 'optimize out' the KFUpdate step.
...
void KMeans::computeClusterMean(
  const std::multimap<int, const Point *> &multimap, int cluster, Point *mean) {
  // Zero-out the mean.
...
void KMeans::printCenters() {
    for ( auto &center_point : center_points_) {
        cout << "Center point: " << center_point << endl;
    }
}

// static
bool KMeans::loadFile(const string &filepath, vector<Point> *points) {
